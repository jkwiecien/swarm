...
export function delegationGuardLines(enabled: boolean): readonly string[] {
	...
	return [
		...
		'Only delegate when it will actually pay off. You still spend tokens writing the contract and',
		'reviewing the result, and the child spends its own — so it is worth it ONLY when the mechanical',
		'application is large relative to the decision: the facts and exact wording are already decided and',
		'the same change is applied across several files/sections (a high apply-to-decide ratio). Prefer to',
		'do it yourself when the edit spans only one or two spots, when anything about WHAT to write is',
		'still undecided, or when specifying the contract would be as much work as just making the edit —',
		'in those cases delegation costs more than it saves. When unsure, do not delegate.',

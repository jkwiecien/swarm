 ...
 * (`pr-review-submitted`); and invalidating a preplanned split child while it
 * remains in Planning starts fallback Planning (`preplan-invalidated`). Each
 * handler resolves the phase's inputs and the
 ...
 * (`projects_v2_item` vs `issues` vs `pull_request` vs `check_suite` vs
...
import { createPreplanInvalidatedTrigger } from './handlers/preplan-invalidated.js';
...
export function registerBuiltInTriggers(registry: TriggerRegistry): void {
	...
	registry.register(createPreplanInvalidatedTrigger());

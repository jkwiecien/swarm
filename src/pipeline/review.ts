...
export function buildReviewPrompt(context: {
	...
	return [
		...
		`4. Submit exactly ONE formal review, non-interactively: write the review body to a scratch file (e.g. review_body.md), then run \`gh pr review ${prNumber} --repo ${repo} --body-file <file>\` with exactly one of \`--approve\` (nothing worth blocking on), \`--request-changes\` (correctness bugs or other issues that must be fixed before merge), or \`--comment\` (non-blocking questions/suggestions only).`,
		`5. Write the verdict you submitted — exactly one of \`approve\`, \`request-changes\`, or \`comment\`, and nothing else — to a file named "${REVIEW_VERDICT_FILENAME}" at the root of this worktree. Do NOT \`git add\`/commit this file (or the body scratch file) — they are scratch hand-offs read by SWARM, not part of the PR.`,
		'',
...
export async function runReviewPhase(options: RunReviewPhaseOptions): Promise<ReviewPhaseResult> {
	...
	logger.info('review phase: start', { taskId, prNumber, headSha, cli });

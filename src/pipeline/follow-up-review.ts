 ...
 * (`src/pipeline/respond-to-review.ts`) the same way `enableAutoMergeIfEligible`
 * is injected into `runReviewPhase` (`src/pipeline/auto-merge.ts`): a typed
 * operation with a real GitHub/queue-backed default, overridden in tests.
 *

 ...
 * (GitHub Projects items have no long-form body of their own). The item stays
 * in "Planning" — a human reviews the plan, then moves the item to "ToDo"
 * themselves to greenlight Implementation (`src/pm/pipeline.ts`); this phase
 * never moves it on its own. The plan is a review artefact, not code — it's
 * delivered as a comment and the worktree is thrown away, so the checkout is
 * detached and never commits.
 *
...
export interface RunPlanningPhaseOptions {
	 ...
	 */
	taskId: string;
	/** PM provider used to post the plan comment. */
	pm: PMProvider;
 ...
 * tells the human how to advance the pipeline (moving the item to "ToDo" is
 ...
 * the linked Issue. The item stays in "Planning" — this phase never moves it;
 * a human moves it to "ToDo" after reviewing the plan.
 *

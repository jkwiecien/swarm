 ...
 *
 * Scope (issue #117): the first — and, for now, only — setting is
 * `agents.defaults`, the *global* per-CLI default model, resolved by the worker
 * as the tier between a project's own `agents.defaults` and the coded default
 * (`src/worker/consumer.ts` `resolveModel`). The top-level object shape is
 * deliberate so future global settings (host URL, worker concurrency, …) drop
 * in as sibling keys without a migration (the blob is extensible).
 */
...
/**
 * The whole global-settings blob. Every key is optional so an empty `{}` is a
 * valid (all-defaults) settings object, and new global settings can be added as
 * sibling keys later without reshaping what's stored.
 */
...
/**
 * The empty baseline returned when no `app_settings` row exists. Intentionally
 * `{}`: with no global overrides, the coded per-CLI defaults still apply
 * downstream via `DEFAULT_MODEL_PER_CLI`, so this is a correct "nothing
 * configured" value, not a partial one.
 */

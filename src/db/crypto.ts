...
/**
 * Credential encryption at rest — mirrors Cascade's `src/db/crypto.ts`.
 *
 * Secrets are encrypted with AES-256-GCM before being stored in
 * `project_credentials`, using the owning `projectId` as Additional
 * Authenticated Data (AAD) so a ciphertext can't be replayed under a different
 * project. Encryption is opt-in: with no `CREDENTIAL_MASTER_KEY` set, values
 * pass through as plaintext (dev/local default), and the `enc:v1:` prefix lets
 * both modes coexist in one column during a rollout.
 */
 ...
 * @param aad - Additional Authenticated Data (projectId) to bind the ciphertext to the project.
 */

...
| `pipeline.respondToReview.autoMerge` | `false` | If true, SWARM enables GitHub auto-merge whenever its own review is satisfied: when the Review phase submits an `approve`, or after Respond-to-review fixes findings / acknowledges a review with no findings. GitHub waits for required checks/reviews or uses the repository merge queue; `request-changes` and pushbacks on concrete review points never enable it. An unavailable auto-merge is logged but does not fail the run. |

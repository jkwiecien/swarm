...
- **Queue**: BullMQ on Redis — retries, backoff, configurable worker-global concurrency (`SWARM_WORKER_CONCURRENCY`, default 1), and a per-project `maxConcurrentJobs` cap. A job blocked only by that internal cap is retained durably with its resolved phase and dispatched as soon as a slot frees; provider rate-limit/capacity failures keep their bounded delayed backoff. PR review-lifecycle jobs (`pull_request`/`pull_request_review`/`check_suite`) always queue ahead of PM-board jobs (`projects_v2_item`, which drive Planning/Implementation) regardless of those limits, so a review never sits behind a multi-minute implementation run (`src/queue/producer.ts`'s `priorityFor`). When `pipeline.prioritizeContinuations` is on (the default), an SCM-driven continuation (Review, Respond-to-review, Respond-to-CI, or Resolve-conflicts) is selected ahead of new board work at that slot release.
...
3. **Review** (Claude Code, reviewer persona) — PR opened / checks pass → the agent writes a structured verdict and review body; SWARM submits the formal review under the reviewer persona. When `pipeline.respondToReview.autoMerge` is enabled, a submitted `approve` also requests a merge through the provider-neutral SCM capability: GitHub prefers auto-merge, then makes a head-SHA-guarded direct merge attempt only if repository auto-merge is unavailable. A refusal is visible but never fails the completed review.
...
- `swarm queue clear` (a.k.a. `npm run queue:clear`) — removes all pending BullMQ jobs (waiting, prioritized, and delayed). It deliberately does not cancel an active run; stop the worker first when clearing work before restarting it.

...
| `SWARM_AGENT_TIMEOUT_MS` | `1800000` (30m) | Default wall-clock timeout applied to **every** phase/agent run when the project sets no per-phase `agents.<phase>.timeoutMs` (`src/worker/consumer.ts`). The harness kills a run that exceeds it (SIGTERM, then SIGKILL after a 5s grace) and the run is finalized as `failed` with `timedOut: true`, so a hung agent can't run — or hold a worker slot — indefinitely. A per-phase `timeoutMs` in `swarm.config.json` overrides it. Must be a positive integer or startup throws. |
...
- Runs now record per-phase token usage where the agent CLI reports it (issue #138): a nullable `runs.usage` column, populated from `claude -p --output-format json` and `codex exec --json` (`src/harness/usage.ts`) and shown on the runs table and run-detail page. Antigravity has no structured-output flag, so its runs — like every pre-existing run — render a graceful "not reported" state.

...
| `pipeline.prioritizeContinuations` | `true` | When a Review that continues in-flight work is blocked only by `maxConcurrentJobs`, it is retained as a pending continuation (with a visible `deferred` run row) and promoted ahead of new Planning/Implementation work once a slot frees, instead of waiting out the generic retry backoff. Set `false` to keep the prior FIFO/backoff behavior. (Wired for the Review continuation for now; the other SCM-driven continuations follow.) |

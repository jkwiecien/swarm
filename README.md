...
| `pipeline.respondToReview.autoMerge` | `false` | If true, SWARM enables GitHub auto-merge once the Review phase submits an `approve` — the only outcome that clears the review gate (issue #235). GitHub waits for required checks/reviews or uses the repository merge queue; `request-changes`, and Respond-to-review's own `fixed`/`pushed-back`/`no-findings` outcomes, never enable it — addressing feedback isn't the same as a fresh approval. An unavailable auto-merge is logged but does not fail the run. |
| `pipeline.respondToCi.enabled` | `true` | If false, failed-check events are skipped without dispatching Respond-to-CI. |
| `pipeline.prioritizeContinuations` | `true` | When Review, Respond-to-review, Respond-to-CI, or Resolve-conflicts is blocked only by `maxConcurrentJobs`, it is retained as a pending continuation (with a visible `deferred` run row) and promoted ahead of new Planning/Implementation work once a slot frees, instead of waiting out the generic retry backoff. Set `false` to keep the prior FIFO/backoff behavior. |

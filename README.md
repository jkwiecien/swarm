...
- **Projects** — create a project (`id`, `name`, `repo`, `repoRoot`) and delete one, plus per-project **General Settings** (`repo`, `repoRoot`, `worktreeRoot`, `baseBranch`, `branchPrefix`), **Agent Configuration** (per-phase `cli`/`model`/`reasoning`/timeout and the project-level `agents.defaults`), and the Respond-to-review auto-merge/minor-review controls.

...
- Runs record per-phase token usage where the agent CLI reports it (issue #138) and curated child observations in `runs.delegations`: parent run/session and phase, child/model, contract type and paths, duration/usage, outcome, and the primary's accepted/reworked disposition. Antigravity has no structured-output flag, so its run usage remains gracefully unavailable.

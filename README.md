...
Worker-spawned agents are scoped to exactly one phase. Their prompts prohibit the manual `solve-issue` skill, arbitrary subagents/skills, and continuing into another phase; the worker owns every phase hand-off and supplies the correct persona and worktree. An opted-in Claude phase may invoke only the project-owned `swarm-doc-editor` under a validated, exact-path delegation contract. The contract is accepted only from the pending native Agent call (never child/tool output), canonical paths cannot escape through symlinks, and the child has a fixed provider-enforced 12-turn limit. The Haiku child cannot run commands or delivery/GitHub operations, and the primary must inspect and accept or rework each unique child invocation. The manual skill remains available for explicitly requested interactive sessions outside the worker.
...
| `SWARM_NATIVE_DELEGATION_ENABLED` | `true` | Global emergency switch for curated native semantic delegation. The literal `false` disables it for every project/phase; projects remain opted out unless `agents.delegation.enabled` and a phase flag are also true. |
...
- **`delegation`** — optional native semantic-delegation policy. It is off unless `enabled: true`; `model` is pinned to `haiku`; `minimumSemanticOperations` defaults to `3` and cannot be lower; and `phases` is a boolean map using the phase keys below (default `{ implementation: true }`). Both this project policy and the global environment switch must allow a Claude phase. The target project must own `.claude/agents/swarm-phase-coordinator.md`, `.claude/agents/swarm-doc-editor.md`, and its contract hooks; SWARM's definitions are the reference implementation.
...
- **Projects** — create a project (`id`, `name`, `repo`, `repoRoot`) and delete one, plus per-project **General Settings** (`repo`, `repoRoot`, `worktreeRoot`, `baseBranch`, `branchPrefix`), **Agent Configuration** (per-phase `cli`/`model`/timeout and the project-level `agents.defaults`), and the Respond-to-review auto-merge/minor-review controls.
...
- Runs record per-phase token usage where the agent CLI reports it (issue #138) and curated child observations in `runs.delegations`: parent run/session and phase, unique child invocation, child/model, contract type and paths, duration/usage, outcome, and the primary's invocation-scoped accepted/reworked disposition. Delegation event, review, and start records are scratch files excluded from deterministic delivery. Antigravity has no structured-output flag, so its run usage remains gracefully unavailable.

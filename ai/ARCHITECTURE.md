...
A retry **reuses the originating `runs` row** rather than inserting a second one (issue #136): the `phase-deferred` outcome carries the run's `runId`, `reenqueueDeferred` threads it onto the re-enqueued job (`jobBase.runId` in `src/queue/jobs.ts`), and on dequeue `processJob` resets that row to `running` (clearing `completedAt`/`error`/`nextRetryAt` and the outcome columns) instead of creating a new one — so an automatic *or* manual retry shows as one run whose status flips, not two. A **manual "Retry now"** (the `runs.retryNow` tRPC mutation, offered on the dashboard for a `deferred` run) fires that pending retry immediately: `promoteRetryForRun` (`src/queue/producer.ts`) locates the run's delayed BullMQ job by its `runId`, resets its `rateLimitRetryAttempt` to 0 (bypassing the automatic cap), and `promote()`s it to run now. Operating on that single pending job is also the duplicate guard — a manual retry can't race the automatic one into two concurrent runs.

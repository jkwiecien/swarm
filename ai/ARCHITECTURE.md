...
Dashboard (Hono + tRPC API, host process)
   — serves /health and /trpc (scaffold)
   — self-hosted single-process model for local-first scope
```

Redis (for BullMQ) and Postgres (for project config, credentials at rest, and run history — same role it plays in Cascade) run in the same Docker Compose stack as the router. The **worker** and the **dashboard** are the exceptions: they run directly on the host (the worker to provision Git worktrees and spawn the `claude` / `antigravity` CLIs which need local PATH/auth, and the dashboard for local-first host run), reaching Redis/Postgres over their published host ports. There is no separate "cloud" process for the MVP; router, worker, dashboard, Redis, and Postgres are all local.

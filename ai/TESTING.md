...
- **Integration tests** (`tests/integration/**/*.test.ts`) — run against a real, ephemeral Postgres (Docker Compose, same `npm run test:db:up` pattern as Cascade) and a real Redis. Run serially, not in parallel, to avoid state collisions.

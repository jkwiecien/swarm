...
| `SWARM_WORKER_CREDENTIAL` | _(unset)_ | Raw registered-worker credential printed once by `swarm workers register`. When set, the host worker authenticates, acquires and heartbeats that worker's fenced session, and may execute only dispatches atomically selected/capacity-claimed for that worker. An unset process may serve projects with no enrollments (unfederated mode) but defers every federated dispatch; an invalid credential or already-live session makes startup fail. Never commit this value. |
...
| `SWARM_WORKER_HEARTBEAT_TTL_MS` | `60000` (60s) | How long a worker session lease stays live after its last heartbeat (`src/identity/worker-session-service.ts`). The host worker refreshes it at one third of this interval. An older session is expired and may be re-acquired with a bumped fencing token; dispatch execution atomically validates that token and session before reserving capacity, so a stale/replaced host is rejected. Must be a positive integer or the operation throws. |
